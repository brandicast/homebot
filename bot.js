...
bot.on('leave', function (event) {
    logger.debug(event);
    log_msg = MEMBERS[event.source.userId]["displayName"] + " 離開麵包國了 !"
    logger.info (log_msg) ;
    mqtt_agent.publish(config.mqtt.topic_for_homebot, log_msg) ;
...
bot.on('join', function (event) {
    ...
    event.source.profile().then(function (profile) {
        ...
        event.reply(msg);
        log_msg =  MEMBERS[event.source.userId]["displayName"] + " 加入麵包國了 !" ;
        logger.info (log_msg) ;
        mqtt_agent.publish(config.mqtt.topic_for_homebot, log_msg) ;
    });
})
...
bot.on('follow', function (event) {
    ...
    event.source.profile().then(function (profile) {
        ...
        log_msg =  MEMBERS[event.source.userId]["displayName"] + " 回到麵包國了 !" ;
        logger.info (log_msg) ;
        mqtt_agent.publish(config.mqtt.topic_for_homebot, log_msg) ;
    });
})
...
bot.on('unfollow', function (event) {
    logger.debug(event);
    log_msg =  MEMBERS[event.source.userId]["displayName"] +  " 封鎖麵包國了 !" ;
    logger.info (log_msg) ;
    mqtt_agent.publish(config.mqtt.topic_for_homebot, log_msg) ;
    unregisterMember(event.source.userId);
